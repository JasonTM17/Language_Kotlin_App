...
class AuthRepository {
    ...
    fun findUserByEmail(email: String): UserRecord? =
        transaction {
            Users
                .selectAll()
                .andWhere { Users.email eq email.trim().lowercase() }
                .firstOrNull()
                ?.let(::toUserRecord)
        }
    ...
    fun findRefreshToken(tokenHash: String): RefreshTokenRecord? =
        transaction {
            RefreshTokens
                .selectAll()
                .andWhere { RefreshTokens.tokenHash eq tokenHash }
                .firstOrNull()
                ?.let { row ->
                    RefreshTokenRecord(
    ...
    fun revokeRefreshToken(id: Long) =
        transaction {
    ...
    fun findProfile(userId: Long): ProfileDto? =
        transaction {
    ...
    private fun toUserRecord(row: ResultRow) =
        UserRecord(
            id = row[Users.id],
            email = row[Users.email],
            username = row[Users.username],
