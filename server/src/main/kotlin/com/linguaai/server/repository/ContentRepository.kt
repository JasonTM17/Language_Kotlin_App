...
class ContentRepository {
    ...
    fun findLanguages(): List<LanguageDto> =
        transaction {
     ...
     */
    fun findLanguageById(id: Long): LanguageDto? =
        transaction {
            Languages
                .selectAll()
    ...
    fun findLessonById(id: Long): LessonDto =
        transaction {
            Lessons
                .selectAll()
                .andWhere { Lessons.id eq id }
                .firstOrNull()
                ?.let(::toLesson)
    ...
    fun findVocabularyById(id: Long): VocabularyDto =
        transaction {
            Vocabularies
                .selectAll()
                .andWhere { Vocabularies.id eq id }
                .firstOrNull()
                ?.let(::toVocabulary)
                ...
                QuizQuestions
                    ...
                    .map { row ->
                        QuizQuestionDto(
                            ...
                            options =
                                row[QuizQuestions.options]?.let { json.decodeFromString<List<String>>(it) }
                                    ?: emptyList(),
    ...
    fun submitQuiz(
        ...
        transaction {
            val quiz =
                Quizzes.selectAll().andWhere { Quizzes.id eq quizId }.firstOrNull()
                    ?: throw ApiException(HttpStatusCode.NotFound, ErrorCodes.NOT_FOUND, "Quiz not found")

            val questionRows =
                QuizQuestions
                    .selectAll()
                    .andWhere { QuizQuestions.quizId eq quizId }
                    .orderBy(QuizQuestions.position, SortOrder.ASC)
                    .toList()
            ...
            val weakTopics =
                feedback
                    ...
                    .mapNotNull { item ->
                        questionRows
                            .firstOrNull { it[QuizQuestions.id] == item.questionId }
    ...
    private fun toLessonSummary(row: org.jetbrains.exposed.sql.ResultRow) =
        LessonSummaryDto(
            id = row[Lessons.id],
    ...
    private fun toLesson(row: org.jetbrains.exposed.sql.ResultRow) =
        LessonDto(
            id = row[Lessons.id],
            languageId = row[Lessons.languageId],
            level = row[Lessons.level],
    ...
    private fun toVocabulary(row: org.jetbrains.exposed.sql.ResultRow) =
        VocabularyDto(
