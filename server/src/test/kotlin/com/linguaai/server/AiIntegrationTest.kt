...
class AiIntegrationTest {
    ...
    @Test
    fun `chat without token is unauthorized`() = withApp {
        val chat = client.post("/api/v1/ai/chat") {
            header(HttpHeaders.ContentType, "application/json")
    ...
    @Test
    fun `rate limit returns 429 after quota`() = withApp(rateLimitPerMinute = 2) {
        val token = registerAndLogin()
        repeat(2) {
        ...
        val third = client.post("/api/v1/ai/chat") {
            header(HttpHeaders.Authorization, "Bearer $token")
            header(HttpHeaders.ContentType, "application/json")
    ...
    @Test
    fun `provider timeout maps to 503 ai unavailable`() = withApp(mockScenario = "timeout") {
        val token = registerAndLogin()
        val chat = client.post("/api/v1/ai/chat") {
            header(HttpHeaders.Authorization, "Bearer $token")
            header(HttpHeaders.ContentType, "application/json")
    ...
    @Test
    fun `empty provider response maps to 502`() = withApp(mockScenario = "empty") {
        val token = registerAndLogin()
        val chat = client.post("/api/v1/ai/chat") {
            header(HttpHeaders.Authorization, "Bearer $token")
            header(HttpHeaders.ContentType, "application/json")
    ...
    @Test
    fun `generated quiz validates strict payload`() = withApp {
        val token = registerAndLogin()
        val quiz = client.post("/api/v1/ai/generate-quiz") {
            header(HttpHeaders.Authorization, "Bearer $token")
            header(HttpHeaders.ContentType, "application/json")
    ...
    @Test
    fun `invalid structured output maps to 502`() = withApp(mockScenario = "invalid_json") {
        val token = registerAndLogin()
        val quiz = client.post("/api/v1/ai/generate-quiz") {
            header(HttpHeaders.Authorization, "Bearer $token")
            header(HttpHeaders.ContentType, "application/json")
    ...
    @Test
    fun `sentence correction records a mistake`() = withApp {
        val token = registerAndLogin()
        val correct = client.post("/api/v1/ai/correct") {
            header(HttpHeaders.Authorization, "Bearer $token")
            header(HttpHeaders.ContentType, "application/json")
